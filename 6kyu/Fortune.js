...
function fortune(f0, p, c0, n, i) {
    let currentDeposit = f0
    let withdraw = c0
    for (let loop = 1; loop < n; loop++) {
        currentDeposit = Math.floor(currentDeposit + ((p / 100) * currentDeposit) - withdraw)
        withdraw = Math.floor(withdraw + ((i / 100) * withdraw))
    }
    return currentDeposit > -1 ? true : false
}